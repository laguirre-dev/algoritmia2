...
def seleccionarUniversidad():
    ...
    if not os.path.exists(CARPETA_UNIVERSIDADES):
        ...
        pantalla.red_text(f"Carpeta '{CARPETA_UNIVERSIDADES}' creada. Por favor coloque los archivos JSON ahí.")
        return None

    archivos = [f for f in os.listdir(CARPETA_UNIVERSIDADES) if f.endswith('.json')]
    
    if not archivos:
        pantalla.red_text(f"No se encontraron universidades (JSON) en la carpeta '{CARPETA_UNIVERSIDADES}'.")
        return None
